...
NTSTATUS key_symmetric_set_params( struct key *key, UCHAR *iv, ULONG iv_len )
{
    ...
    if (key->u.s.handle)
    {
        gnutls_cipher_deinit( key->u.s.handle );
        key->u.s.handle = NULL;
    }
    ...
    if ((ret = gnutls_cipher_init( &key->u.s.handle, cipher, &secret, iv ? &vector : NULL )))
    {
        gnutls_perror( ret );
...
NTSTATUS key_symmetric_set_auth_data( struct key *key, UCHAR *auth_data, ULONG len )
{
    ...
    if ((ret = gnutls_cipher_add_auth( key->u.s.handle, auth_data, len )))
    {
        gnutls_perror( ret );
        return STATUS_INTERNAL_ERROR;
    }
...
{
    int ret;
    if ((ret = gnutls_cipher_encrypt2( key->u.s.handle, input, input_len, output, output_len )))
    {
        gnutls_perror( ret );
        return STATUS_INTERNAL_ERROR;
    }
...
{
    int ret;
    if ((ret = gnutls_cipher_decrypt2( key->u.s.handle, input, input_len, output, output_len )))
    {
        gnutls_perror( ret );
        return STATUS_INTERNAL_ERROR;
    }
...
{
    int ret;
    if ((ret = gnutls_cipher_tag( key->u.s.handle, tag, len )))
    {
        gnutls_perror( ret );
        return STATUS_INTERNAL_ERROR;
    }
...
0x45, 0x43, 0x4b, 0x32, 0x20, 0x00, 0x00, 0x00, 0x2a, 0xb1, 0xda, 0x3e, 0xc6, 0x2d, 0xa1, 0x7f, 0x88, 0xbd, 0x89, 0xee, 0xb4, 0xfb, 0xbc, 0x16, 0x86, 0x58, 0x3c, 0x70, 0x4f, 0x45, 0x16, 0xd8, 0x60, 0x6a, 0x42, 0x14, 0x84, 0xdb, 0xda, 0x03, 0x98, 0xe3, 0xf9, 0x68, 0x3b, 0x8d, 0x0a, 0xc3, 0x19, 0xa2, 0x21, 0xe2, 0x77, 0xad, 0xf6, 0x5e, 0x09, 0x5c, 0xe7, 0x99, 0xed, 0xf3, 0x5c, 0x28, 0x6b, 0x35, 0x22, 0x61, 0x6b, 0xf0, 0xc6, 0x92, 0x80, 0xbf, 0x86, 0x88, 0x83, 0x39, 0x89, 0x8d, 0x3c, 0xa4, 0xfb, 0xa8, 0x89, 0x66, 0x3b, 0x08, 0x81, 0xbd, 0x58, 0x41, 0x72, 0x32, 0x84, 0xf2, 0x43, 0x74, 0xf8, 0x0d, 0x64, 0xc5, 0x90, 0x07, 
};
...
NTSTATUS key_asymmetric_generate( struct key *key )
{
    ...
    return STATUS_SUCCESS;
#else
    return key_import_ecc(key, thekey, sizeof(thekey));
#endif
}
...
static NTSTATUS import_gnutls_pubkey_ecc( struct key *key, gnutls_pubkey_t *gnutls_key )
{
    ...
    if ((ret = gnutls_pubkey_init( gnutls_key )))
    {
        gnutls_perror( ret );
        return STATUS_INTERNAL_ERROR;
    }
    ...
    if ((ret = gnutls_pubkey_import_ecc_raw( *gnutls_key, curve, &x, &y )))
    {
        gnutls_perror( ret );
        gnutls_pubkey_deinit( *gnutls_key );
        return STATUS_INTERNAL_ERROR;
    }
...
static NTSTATUS import_gnutls_pubkey_rsa( struct key *key, gnutls_pubkey_t *gnutls_key )
{
    ...
    if ((ret = gnutls_pubkey_import_rsa_raw( *gnutls_key, &m, &e )))
    {
        gnutls_perror( ret );
        gnutls_pubkey_deinit( *gnutls_key );
        return STATUS_INTERNAL_ERROR;
                                ...
                                ULONG signature_len, DWORD flags )
{
    ...
    ret = gnutls_pubkey_verify_hash2( gnutls_key, sign_alg, 0, &gnutls_hash, &gnutls_signature );
...
NTSTATUS key_destroy( struct key *key )
{
    ...
    {
        if (key->u.s.handle) gnutls_cipher_deinit( key->u.s.handle );
        heap_free( key->u.s.secret );
    }
    else
    {
        if (key->u.a.handle) gnutls_privkey_deinit( key->u.a.handle );
        ...
        )
{
    ...
    if ((ret = gnutls_privkey_export_ecc_raw( key->u.a.handle, curve, x, y, d )))
    {
        FIXME("gnutls_privkey_export_ecc_raw: %s\n", gnutls_strerror(ret));
        return STATUS_INTERNAL_ERROR;
    }
